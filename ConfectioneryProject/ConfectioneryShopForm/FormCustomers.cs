...
namespace ConfectioneryShopForm {
    public partial class FormCustomers : Form {
        ...
        private void LoadData() {
            try {
                List<CustomerBindingModel> list =
                    APICustomer.GetRequest<List<CustomerBindingModel>>("api/Customer/GetList");
                if ( list != null ) {
        ...
        private void buttonDel_Click(object sender, EventArgs e) {
            if ( dataGridViewOfCustomers.SelectedRows.Count == 1 ) {
                if ( MessageBox.Show("Удалить запись", "Вопрос", MessageBoxButtons.YesNo,
                    ...
                    try {
                        APICustomer.PostRequest<CustomerBindingModel, bool>("api/Customer/DelElement",
                        new CustomerBindingModel {ID = id});
                    }
                    catch ( Exception ex ) {
                        MessageBox.Show(ex.Message, "Ошибка", MessageBoxButtons.OK,
                        MessageBoxIcon.Error);
